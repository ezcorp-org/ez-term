...
# Usage: curl -sSL https://raw.githubusercontent.com/ezcorp-org/ez-term/main/install.sh | bash
...
case "$OS" in
    ...
    MINGW*|MSYS*|CYGWIN*)
        ...
        echo "Please download from: https://github.com/ezcorp-org/ez-term/releases"
        exit 1
        ;;
    *)
...
LATEST_RELEASE=$(curl -s https://api.github.com/repos/ezcorp-org/ez-term/releases/latest | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')

if [ -z "$LATEST_RELEASE" ]; then
    echo "⚠️  No release found, using direct build from main branch..."
    DOWNLOAD_URL="https://github.com/ezcorp-org/ez-term/releases/download/latest/ez-${PLATFORM}.${ARCHIVE_EXT}"
else
    echo "✅ Latest version: $LATEST_RELEASE"
    DOWNLOAD_URL="https://github.com/ezcorp-org/ez-term/releases/download/${LATEST_RELEASE}/ez-${PLATFORM}.${ARCHIVE_EXT}"
fi
...
if [ -n "$SHELL_WRAPPER" ]; then
    curl -sSL "https://raw.githubusercontent.com/ezcorp-org/ez-term/main/scripts/${SHELL_WRAPPER}" -o "$HOME/.${SHELL_WRAPPER}"
...
echo ""
echo "📚 Documentation: https://github.com/ezcorp-org/ez-term"
echo ""
